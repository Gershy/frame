		...
		var ret = {
			...
			CreativityApp: PACK.uth.makeClass({ name: 'CreativityApp',
				...
				methods: function(sc, c) { return {
					...
					start: function() {
						if (U.isServer()) {
							...
							this.resolveUpdate = new qd.QUpdate({
								request: function(cb) {	pass.tryResolveVote(); cb(null); },
								start: function() {},
								end: function(response) { }
							});
					...
					tryResolveVote: function() {
						var t = this.timeRemaining();
						
						if (t <= 0) this.getChild('resolutionTimer.startedMillis').setValue(-1);
						
						if (this.getChild('votables').length >= 1 && (t === null || t <= 0)) this.resolveVote();
					},
					resolveVote: function() {
						// This function picks the winning votable and adds it to the story
						var ranking = this.votableRanking();
						if (ranking.length === 0) {
							console.log('NUM HERE', this.getChild('votables').length);
					...
					handleQuery: function(params, /* command, params */ onComplete) {
						...
						if (com === 'getToken') {
							...
							*/
							...
							onComplete({ seconds: this.timeRemaining() });
							return;
							...
							build: function(rootElem, subsceneElem) {
								...
								var updateStory = new PACK.quickDev.QUpdate({
									request: function(callback) {
										...
										root.$getChild({ address: 'storyItems', recurse: true }).fire(function(elem) {
										...
										build: function(rootElem, subsceneElem, scene) {
											...
											var updateVotables = new PACK.quickDev.QUpdate({
												request: function(callback) {
													...
													root.$getChild({ address: 'votables', recurse: true }).fire(function(elem) {
