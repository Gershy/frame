  ...
  build: (foundation, record) => {
    ...
    let Lands = U.inspire({ name: 'Lands', methods: () => ({
      ...
      open: async function() {
        ...
        // TODO: HEEERE! Why bother to open all our ways? Just listen to
        // `this.pool.cpuWob`, and create corresponding Huts for each
        // remote cpu. The only hiccup is that the U.Wob instance Below
        // will have already wobbled the single cpu instance for their
        // Above - should take that out of FoundationBrowser (at the
        // same time we decouple it from being inherently Below), and
        // move it here. So a Lands instance Below will know to
        // automatically wobble a single Above instance!
    ...
    let Way = U.inspire({ name: 'Way', methods: (insp, Insp) => ({
      ...
      open: async function() {
        ...
        this.serverFunc = this.server.hold(absConn => {
          ...
          let cpuId = absConn.cpuId;
          let cpu = this.lands.pool.getCpu(cpuId);
      ...
      tellHut: function(hut, msg) {
        let cpu = this.lands.pool.getCpu(hut.cpuId);
        let conn = cpu.serverConns.get(this.server);
    ...
    content.test = rootKeep => rootKeep.contain(k => {
      ...
      k.sandwich.before = async () => {
        ...
        serverWob.spoofClient = () => {
          let cpuId = (cpuIdCnt++).toString(36).padHead(8, '0');
          return lands.pool.makeCpuConn(serverWob, conn => {});
        };
        serverWob.decorateConn = conn => {
          ...
          clients.add(conn);
          conn.shutWob().hold(() => clients.rem(conn));
        };
        
        let way = Way({ makeServer: () => serverWob });
        lands.addWay(way);
        ...
        testData = { trt, server: serverWob, lands, way, clients };
      ...
      U.Keep(k, 'aboveAndBelow').contain(k => {
        ...
        k.sandwich.before = async () => {
          ...
          let addClientBelow = (name, fn=null) => {
            ...
            serverBelow.decorateConn = aboveClient => {
            ...
            serverBelow.spoofClient = () => lands.pool.makeCpuConn(serverBelow, conn => {});
