global.rooms['chess2'] = async foundation => {
  ...
  return Setup('c2', 'chess2', {
    ...
    recForms: {
      'c2.match': U.form({ name: 'Chess2MatchRec', has: { Rec }, props: (forms, Form) => ({
        validMoves: (matchPlayer, piece) => {
          ...
          // Get other pieces
          let pieces = this.relRecs('c2.matchPiece').toArr(matchPiece => matchPiece.mem('piece'));
        ...
        applyMoves: (...playerMoves) => {
          ...
          let matchPieceSet = this.relRecs('c2.matchPiece');
    ...
    parFn: async (hut, rec, real, dep) => {
      ...
      let random = FastRandom(foundation.getMs());
      ...
      dep.scp(rec.relSrc('c2.match'), (match, dep) => {
        ...
        let roundChooser = dep(Chooser(match.relSrc('c2.round')));
        dep.scp(roundChooser.srcs.off, (noRound, dep) => {
          
          let round = hut.createRec('c2.round', [ match ], { ms: foundation.getMs() });
    ...
    kidFn: async (hut, rec, real, dep) => {
      ...
      */
      ...
      dep.scp(playerChooser.srcs.off, (noPlayer, dep) => {
        ...
        dep(enterReal.getLayout(lay.Press).src.route( () => createPlayerAct.act() ));
        
      });
      dep.scp(playerChooser.srcs.onn, (player, dep) => {
        
        let matchPlayerChooser = dep(Chooser(player.relSrc('c2.matchPlayer')));
  ...
  let realWebApp = await foundation.getRoom('realWebApp');
  let term = await foundation.getRoom('term');
